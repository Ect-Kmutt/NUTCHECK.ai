...
async function main() {
  const [students, users, grades, logs] = await Promise.all([
  ...
  const snapshot = { students, users, grades, logs };

  fs.writeFileSync(outPath, JSON.stringify(snapshot, null, 2), "utf8");
  console.log(`Exported snapshot to ${outPath}`);
  console.log(`students=${students.length} users=${users.length} grades=${grades.length} logs=${logs.length}`);
}
